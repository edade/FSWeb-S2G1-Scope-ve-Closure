...
/*Görev 1: macSkoru()
  ...
  skor1'de değişken fonksiyonun içinde tanımlanmış yani skor değişkeni function scope içerisinde bulunuyor ancak skor2'de değişken fonksiyonun dışında yani skor2nin scope dışında tanımlanmış.
...
function skorTabelasi(periyotSkoru, takimSkoru, ceyrekNo) {
  let sonuc = [];
  let EvSahibi = 0;
  let KonukTakim = 0;
  for (let i = 1; i <= ceyrekNo; i++) {
    const period = periyotSkoru(takimSkoru);
    let metin =
      i +
      " .  Periyot: Ev Sahibi " +
      period.EvSahibi +
      ...
      period.KonukTakim;
    sonuc.push(metin);
    EvSahibi += period.EvSahibi;
    KonukTakim += period.KonukTakim;
  }
  let skor =
  ...
  while (EvSahibi === KonukTakim) {
    ...
    KonukTakim = period.KonukTakim;
    skor.push(
      i + " .uzatma EvSahibi " + EvSahibi + " - Konuk Takım " + KonukTakim
    );
    i++;
  }
